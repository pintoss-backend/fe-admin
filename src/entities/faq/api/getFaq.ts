...
let FAKE_DB: Faq[] = [...seedFaqs];

export async function listFaqsMock(): Promise<Faq[]> {
	return Promise.resolve(FAKE_DB);
}

export async function createFaqMock(input: FaqCreateInput): Promise<Faq> {
...
export async function deleteFaqMock(id: string): Promise<void> {
	FAKE_DB = FAKE_DB.filter((f) => f.id !== id);
