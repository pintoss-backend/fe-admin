import { Typography, Card, Table, Button, Space, Popconfirm, message, Empty, Image } from 'antd';
import {
	DeleteOutlined,
	PlusOutlined,
	EditOutlined,
...
export const ProductList: React.FC<ProductListProps> = ({
			...
			},
			{
				...
				render: (_: any, record: TreeDataNode) => {
					...
					if (record.isParent) {
						...
						const parentRecord = treeData.find(
							(item) => item.isParent && item.children?.some((child) => child.key === record.key),
						);
			...
			},
			{
				...
				render: (_: any, record: TreeDataNode) => {
					...
					if (record.isParent) {
						...
						return <div className={styles.priceText}>{(record.price || 0).toLocaleString()}원</div>;
					}
				},
			},
			{
								...
								<>
									<Button
										...
										onClick={() => {
											if (onEditIssuer) {
												// 전체 발행사 정보를 찾아서 전달
												const fullIssuerData = issuers.find(
													(issuer) => issuer.id === record.issuerId,
												);
	...
	return (
		<div className={styles.productListCard}>
			<Card title="상품 목록" extra={extraButtons}>
				{treeData.length === 0 ? (
					<Empty
