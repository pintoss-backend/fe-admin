...
export const IssuerForm: React.FC<IssuerFormProps> = ({ visible, onSubmit, onCancel, initialData }) => {
	const [form] = Form.useForm();
	const [paymentMethods, setPaymentMethods] = React.useState<Array<{ paymentMethod: string; discountRate: number }>>([
	...
	const [uploadedFileMetadata, setUploadedFileMetadata] = React.useState<{ name: string; size: number } | null>(null);
	...
	const handlePaymentMethodChange = (index: number, field: 'paymentMethod' | 'discountRate', value: string | number) => {
		setPaymentMethods(prev => prev.map((item, i) => 
	...
	const handleFormChange = (changedValues: any) => {
		...
		// phoneNumber는 선택사항이므로 입력하면 다음 단계로, 아니면 건너뛰기 버튼 사용
		if (changedValues.phoneNumber && currentStep === 2) {
			setCurrentStep(3);
			scrollToBottom();
		}
		// homepage는 선택사항이므로 입력하면 다음 단계로, 아니면 건너뛰기 버튼 사용
					...
					>
						<div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
							...
							>
								...
								<p className="ant-upload-text" style={{ color: uploadedImageUrl ? '#52c41a' : '#666' }}>
									{uploadedImageUrl ? '새 이미지로 교체하려면 클릭하거나 드래그하세요' : '클릭하거나 파일을 드래그하여 업로드'}
								</p>
							...
							{paymentMethods.map((pm, index) => (
								<div key={index} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
									<Input
										...
										onChange={(e) => handlePaymentMethodChange(index, 'paymentMethod', e.target.value)}
										style={{ flex: 1 }}
									/>
									<InputNumber
										placeholder="할인률"
										...
										onChange={(value) => handlePaymentMethodChange(index, 'discountRate', value || 0)}
										style={{ width: 100 }}
									/>
									<Button 
										type="text" 
